...
const TecladosTot=()=>{
    ...
    const filtrar=()=>{
        Axios.post("http://localhost:3000/teclados/filt",{per_tec:per
        }).then((response)=>{
            console.log(response)
            setCont(response.data.teclado.length)
            ...
            <div key={r.id_tor} id="barra">
                <table >
                    <tbody >
                        <tr>
                        <td className="tabb">{r.eti_tec}</td>
                            <td className="tabb">{r.mar_tec}</td>
                            <td className="tabb">{r.per_tec}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    useEffect(() => {
        Axios.get("http://localhost:3000/teclados/all/").then((response) => {
            console.log(response)
            setCont(response.data.teclado.length)
            const resp = response.data.teclado
            const listItems = resp.map((r) =>
            <div key={r.teclado.id_tec} id="barra">
                <table >
                    <tbody >
                        <tr>
                            <td className="tabb">{r.teclado.eti_tec}</td>
                            <td className="tabb">{r.teclado.mar_tec}</td>
                            <td className="tabb">{r.teclado.per_tec}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
